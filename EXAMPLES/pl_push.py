...
while True:
    ...
    for event in fixture["events"]:
        if event in events:
            ...
            if event["type"] == "G":
                player = pl.player(event["personId"])
                for club in fixture["teams"]:
                    if club["team"]["id"] == event["teamId"]:
                        team = club["team"]["club"]["abbr"]
                title = "GOOOAAL! "+pl.player(event["personId"])["name"]["display"]+" ("+team+") - "+event["clock"]["label"][:-2]
            elif event["type"] == "PS":
                ...
                for club in fixture["teams"]:
                    if club["team"]["id"] == event["teamId"]:
                        team = club["team"]["club"]["abbr"]
                title = "SUB "+event["description"]+" - "+pl.player(event["personId"])["name"]["display"]+" ("+team+") - "+event["clock"]["label"][:-2]
            elif event["type"] == "B":
                try:
                    x = event["personId"]
                    player = pl.player(event["personId"])
                    ...
                    if event["description"] == "Y":
                        title = "YELLOW CARD - "+player["name"]["display"]+" ("+team+") - "+event["clock"]["label"][:-2]
                    elif event["description"] == "R":
                        title = "RED CARD - "+player["name"]["display"]+" ("+event["clock"]["label"][:-2]+")"
                except KeyError:
